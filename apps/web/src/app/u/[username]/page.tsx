...
const ReadOnlyGridItem = ({ item }: { item: GridItem }) => {
    ...
    return (
        <div style={style} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm overflow-hidden relative h-full w-full hover:shadow-md transition-shadow">
             {item.type === 'text' && (
            ...
            {item.type === 'app' && (
                <div className="flex flex-col items-center justify-center h-full w-full p-4">
                    <div className="w-12 h-12 bg-blue-500 rounded-xl mb-2 flex items-center justify-center text-white">
                        <svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 10l-2 1m0 0l-2-1m2 1v2.5M20 7l-2 1m2-1l-2-1m2 1v2.5M14 4l-2-1-2 1M4 7l2-1M4 7l2 1M4 7v2.5M12 21l-2-1m2 1l2-1m-2 1v-2.5M6 18l-2-1v-2.5M18 18l2-1v-2.5" /></svg>
                    </div>
                    <span className="text-xs font-bold text-center line-clamp-2">{item.content}</span>
                </div>
            )}
            {item.type === 'video' && (
                <VideoEmbed url={item.content} />
            )}
            {item.type === 'social' && (
                <a 
...
export default async function PublicProfilePage({ params }: PageProps) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col">
            <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-4 text-center sticky top-0 z-50 shadow-sm">
            ...
            <div className="flex-1 w-full max-w-4xl mx-auto p-4">
                ...
                >
                    {items.map(item => (
                        <ReadOnlyGridItem key={item.id} item={item} />
                    ))}
                </div>
            </div>

            <div className="p-4 text-center text-sm text-gray-500">
                Powered by <a href="/" className="underline hover:text-gray-800 dark:hover:text-gray-300">Yooka Builder</a>
            </div>
        </div>
    );
}
